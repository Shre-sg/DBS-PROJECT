...
router.post('/', (req, res) => {
    ...
    // Ensure that the Div_Name is always set to "propulsion"
...
router.get('/', (req, res) => {
    // Fetch students with Div_Name "propulsion"
    db.query('SELECT * FROM STUDENT WHERE Div_Name = ?', 'propulsion', (err, students) => {
        if (err) {
            console.error('Error fetching students:', err);
            return res.status(500).json({ error: 'Internal server error' });
        }

        // Fetch COMPONENT and TRANSACTION details for students with Div_Name "propulsion"
        db.query('SELECT * FROM COMPONENTS WHERE Student_ID IN (SELECT USN FROM STUDENT WHERE Div_Name = ?)', 'propulsion', (err, components) => {
            if (err) {
                console.error('Error fetching components:', err);
                return res.status(500).json({ error: 'Internal server error' });
            }

            db.query('SELECT * FROM TRANSACTION WHERE Product_ID IN (SELECT Component_ID FROM COMPONENTS WHERE Student_ID IN (SELECT USN FROM STUDENT WHERE Div_Name = ?))', 'propulsion', (err, transactions) => {
                if (err) {
